...
BEGIN 
	...
	select  'IT10' as ID_AZIENDA,
		...
		END DESC_STATO_CLIENTE,
			...
			a.CAP as CAP,
			CASE WHEN a.Localita is Null THEN GALA_CB.GetIndirizzoCliente (a.IDAnagrafica) else a.Localita END as COMUNE,			
...
SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	...
	SELECT  distinct
		A.IDTBilling, A.IDFattura, A.[Numero_Documento], A.Emissione, Scadenza, [Scadenza_Ricalcolata], ES.DataOraLetturaEsito
		into #FattureFEPA_OK
	...
	SELECT  distinct
		A.IDTBilling, A.IDFattura, A.[Numero_Documento], A.Emissione, Scadenza, [Scadenza_Ricalcolata], DataOraLetturaEsito
		into #FattureFEPA_OK_UPD
		FROM		#FattureFEPA_OK as A
		WHERE		DataOraLetturaEsito between @DataDa and @DataA
		



Select distinct * into #tmpMovimenti from(
select 	m.idMovimento as Id_Record,
		...
		CASE WHEN c.idCausale in (1,7) THEN fepa.Scadenza_Ricalcolata ELSE null END as DATA_SCAD_RICALCOLATA
...
left join dbo.TipiPagamento tp on tp.IDTipoPagamento = doct.idMetodoPag
left join GALA_CB.GALA_DESCRIZIONE_TIPO_FATTURA gdf on s.TipoDoc = gdf.IdTipoDOC
inner join #FattureFEPA_OK_UPD fepa on fepa.IDFattura = m.IDFattura 
where	a.IDStatoAnagrafica=1
		and m.IDAnagrafica!='100001'
		and m.IDStato>=0
		and exists (select	1 
...
DROP TABLE #FattureFEPA_OK
DROP TABLE #FattureFEPA_OK_UPD
DROP TABLE #FattureFEPA
...
BEGIN
	...
	--	AND A.[Scadenza_Ricalcolata] Is Null) fepaKO
		...
		SELECT DISTINCT * INTO #FattureFEPA_OK_Dis  FROM #FattureFEPA_OK
	...
	select    s.IDAnagrafica,
		...
		from      Tes.Movimenti m
			...
			LEFT JOIN BillingTGALA.dbo.DocT bill ON s.IDTBilling = bill.IDDocT
			LEFT JOIN #FattureFEPA_OK_Dis fok ON m.idFattura = fok.idFattura
		where  m.IDAnagrafica=s.IDAnagrafica 
				...
				and exists (select	1 
							...
							where	contr.IDAnagrafica= m.IDAnagrafica
								and cr.IDStatoRiga != 11)
                and COALESCE(fok.Scadenza_Ricalcolata,s.DataScad) <= @DataA
                and m.DatPMO <= @DataA
...
PRINT '17) Creazione Store Procedure CB_Sp_getFattureEscluse'
GO

IF EXISTS (SELECT * FROM sysobjects WHERE id=OBJECT_ID('GALA_CB.CB_Sp_getFattureEscluse')) 
	DROP PROCEDURE GALA_CB.CB_Sp_getFattureEscluse
GO

CREATE PROCEDURE [GALA_CB].[CB_Sp_getFattureEscluse]
	@DataDa Datetime,
	@DataA  Datetime,
	@IdCliente INT
AS
BEGIN
	...
	select distinct fko.IdFattura as ID_FATTURA, 
	(SELECT IdAnagrafica from scadenzario 
	where IDFattura = fko.idFattura) as ID_CLIENTE,
	'FEPA_KO' as MOTIVAZIONE
	INTO #FattureEscluse
	from #FattureFEPA_KO fko
	

	-- SE UN FLUSSO DA ESCLUDERE E' ANCHE FEPA KO, LO VISUALIZZO COME FEPA KO
	insert into #FattureEscluse (ID_FATTURA, ID_CLIENTE, MOTIVAZIONE)
			select  distinct  s.idFattura,
							  s.idAnagrafica,
							  'ESCLUSA'
			from      Scadenzario s 
			inner JOIN BillingTGALA.dbo.DocT bill ON s.IDTBilling = bill.IDDocT 
			where  isnull(bill.IDDocFlusso,0) in (select A.IDDocFlusso FROM dbDatamaxGALA.dbo.FlussoDaEscludereScadenzario A)
			and Not exists (SELECT * FROM #FattureEscluse fe where fe.ID_FATTURA = s.idFattura)


	SELECT * FROM #FattureEscluse
	
	--Drop Temp Table--
	drop table #FattureFEPA
	drop table #FattureFEPA_KO
	drop table #FattureEscluse
